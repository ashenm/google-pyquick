...
def read_urls(filename):
  ...
  requests = {}
  protocol = 'http://'
  hostname = filename.partition('_')[-1]

  with open(filename, 'rU') as hits:
    for hit in hits:
      match = re.search(r'\S+/puzzle/\S+', hit)
      if match:
        requests[protocol + hostname + match.group()] = True

  return sorted(requests.keys(), key=sort_url)
